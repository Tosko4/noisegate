...
`reduce-json` treats the complete serialized envelope as its artifact transaction boundary. With artifact mode enabled it first computes a no-write preview, accepts only a smaller final envelope, and only then performs the artifact-writing pass. An outer no-gain decision therefore returns the original payload without creating an orphaned raw artifact or a recovery ID that was never delivered.
